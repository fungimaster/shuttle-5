	...
	<div>
		<b-container class>
          ...
          <li>{{ hole.hole }} : {{ hole.slag }}</li>
						...
						<button @click="sendWinner">SEND WINNER</button>
						<button @click="getGameData">GET GAME DATA</button>
	...
	export default {
		...
		methods: {
			async getGameData() {
				...
				const teamId = {
					id: "E5zWJ6wqnZNNCkY2i"
				};

				try {
					let response = await axios.post(url, teamId);
			...
			async sendWinner() {
				...
				const data = {
					_id: "E5zWJ6wqnZNNCkY2i",
					holes: [{ hole: 12, par: 5, index: 18, slag: [9, 9, 3, 4] }]
				};
